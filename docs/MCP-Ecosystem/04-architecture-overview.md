...
5. **All-or-nothing tool access.** The MCP specification includes OAuth 2.1 authorization at the server level, but there is no built-in mechanism for per-tool access control. MCP's native `tools/list` returns every tool a server exposes to any authenticated client. Either a user has access to the server and all its tools, or they have access to none.  
...
6. **All-or-nothing tool access → AuthPolicies with CEL predicates \+ MCPVirtualServers.** AuthPolicies define per-tool or per-server authorization rules using CEL expressions evaluated against JWT claims (groups, roles, subject). MCPVirtualServers further filter `tools/list` responses so users only see the tools relevant to their role. Together, they provide fine-grained, identity-based tool access control.  
...
11. **LLMs degrade with too many tools → MCPVirtualServers for tool curation.** MCPVirtualServers allow platform or AI engineers to define narrow, purpose-specific tool subsets. An LLM agent for code review might see only three tools rather than the full set of thirty available in the namespace — reducing token consumption and improving tool-selection accuracy.  
