...
* **AI engineer usage workflows.** Catalog browsing, MCP server deployment via the UI and `MCPServer` CR, gateway registration (HTTPRoute \+ MCPServerRegistration), MCPVirtualServer creation, Playground interaction with tool calling, and adding custom servers to the catalog (section 5.2).  
