# Module 3: Prepare MCP Server Prerequisites
...
> Until Module 7 (External OIDC) and Module 8 (AuthPolicies, passthrough),
> the server runs as the `mcp-viewer` ServiceAccount, whose `view` role
> cannot write — so enabling write tools here does not open a write path
> by itself.
...
| ConfigMap | mcp-ecosystem | Server config: writes on / destructive off, denied resources, toolsets |
