# Module 18: Gen AI Playground with MCP Tools (Optional Model Track)
...
- Core path (Modules 0--8) complete (MCP Gateway with authentication)
...
> **Working directory:** `cd deploy/workshop/18-playground`
...
!!! note "One Gateway URL"

    The Playground uses the same client URL as every other consumer:
    `https://mcp-gateway.<CLUSTER_DOMAIN>/mcp` (created in Module 2). Do
    not point it at in-cluster services — the broker service bypasses
    ext_proc/AuthPolicy, and per-server hostnames no longer exist.
...
```bash
sed "s/<CLUSTER_DOMAIN>/${CLUSTER_DOMAIN}/g" mcp-servers-configmap.yaml \
  | oc apply --context="$CTX" -f -
```
...
5. Click the **View tools** icon to see the available MCP tools (should match what Module 17 showed)
...
> filtering from Module 8 applies.
