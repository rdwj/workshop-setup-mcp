...
- **The Authentication CR can't be `oc apply`d to OIDC directly.** The installer leaves `spec.webhookTokenAuthenticator` set, which is invalid with `type: OIDC`; remove it with a JSON patch first.
