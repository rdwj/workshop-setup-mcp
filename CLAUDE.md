...
- **Gateway tool filtering is visibility-only, not execution-enforced.** The wristband (OPA Rego → `x-authorized-tools`) correctly filters `tools/list` responses (39→23 tools for mcp-users). However, `tools/call` for hidden tools is NOT blocked — the call is forwarded to the backend. This is because MCPVirtualServer routing (which would enforce call-level access) is non-functional due to filter chain ordering (lesson #4: ext_proc runs before the Kuadrant Wasm plugin injects the `x-mcp-virtualserver` header). Backend RBAC (e.g., Kubernetes RBAC on the ServiceAccount) is the real security boundary. E.g., `openshift_events_list` succeeds for all users because the SA has cluster-wide event read access.
