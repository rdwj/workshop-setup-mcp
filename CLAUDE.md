...
- **Gateway tool filtering is visibility-only, not execution-enforced.** The OPA Rego policy in the AuthPolicy filters `tools/list` responses (hiding tools from unauthorized groups) but does NOT block `tools/call` for hidden tools. A client that knows a hidden tool's name can call it directly. Backend RBAC (e.g., Kubernetes RBAC on the ServiceAccount) is the real security boundary. This matters when the backend SA has broader permissions than the user should have — e.g., `openshift_events_list` succeeds for all users because the SA has event read access regardless of group membership.
